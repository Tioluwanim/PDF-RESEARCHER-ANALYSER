        ...
        for msg in history:
            if msg.role == MessageRole.USER:
                ...
                chat_html += (
                    ...
                    f'<div class="msg-bubble asst">{_md_to_html(msg.content)}</div>'
                    '</div>'
                )
        chat_html += '</div>'
    ...
    try:
        ...
        for token in stream:
            ...
            container.markdown(
                '<div class="msg-wrap asst">'
                '<div class="msg-avatar asst">📚</div>'
                f'<div class="msg-bubble asst">{_md_to_html(full_reply)}'
        ...
        typing.empty()
        container.empty()  # clear streaming view — full rerun will render history
...
def _render_batch_tab() -> None:
    ...
    # Preview list (first 15) — read size safely
    def _file_size_kb(f) -> str:
        try:
    ...
    preview_rows = "".join(
        ...
        f'color:var(--muted);">{_file_size_kb(f)}</span>'
        f'</div>'
